...
Eigen::VectorXd Controller::WBC(double t)
{
    ...
    //std::cout<< qppRef << std::endl;
    Eigen::VectorXd hGpRef = PDMomentumAcc();
    //std::cout<< hGpRef << std::endl;
    ...
    Eigen::MatrixXd H = Eigen::MatrixXd::Zero(42, 42); //Hessian qp
    ...
    Eigen::VectorXd g = Eigen::VectorXd::Zero(42);
    hGpRef << 0,0,0,0,-1.176944810198006,0;
...
{   
    ...
    Aeq(0,robot_.getNumJoints() + 3 + 1) = -1; //-fy + c11*v1y + c12*v2y + c13v3y + c14v4y + ... = 0
    ...
    Aeq(0,robot_.getNumJoints() + 3 + 2) = -1; //-fz + c11*v1z + c12*v2z + c13v3z + c14v4z ... = 0
...
{
    Eigen::VectorXd qpp(42);
    Eigen::MatrixXd Hsym = 0.5 * (H + H.transpose()); //guarantee symmetric and positive definite

    //Dynamics constraint
    Eigen::MatrixXd A1 = Eigen::MatrixXd::Zero(numDynamicsEqConstraints_, 42); // Dynamics constraint M /dot{v} - J^T*f = -C 
    ...
    //std::cout<<A1.block(0,0,6,12)<< std::endl<<std::endl;
    //std::cout<<A1.block(0,30,6,12)<< std::endl;
    Eigen::VectorXd b1 = -dyn_.getC().segment(0,numDynamicsEqConstraints_);
    //std::cout<<b1<<std::endl;

    //Friction constraints
    Eigen::MatrixXd Aeq = Eigen::MatrixXd::Zero(numFrictionEqConstraints_,42); 
    ...
    //Friction
    A.block(numIneqConstraints_, 0, numFrictionEqConstraints_, numDesVariables_) = Aeq;
    lbA.segment(numIneqConstraints_, numFrictionEqConstraints_) = beq;
    ubA.segment(numIneqConstraints_, numFrictionEqConstraints_) = beq; 
    //Dynamics
    A.block(numIneqConstraints_ + numFrictionEqConstraints_, 0, numDynamicsEqConstraints_, numDesVariables_) = A1;
    lbA.segment(numIneqConstraints_ + numFrictionEqConstraints_, numDynamicsEqConstraints_) = b1;
